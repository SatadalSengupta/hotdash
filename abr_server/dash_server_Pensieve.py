...
def mlog(fnc="none", msg=""):
    print msg
    master_log(pkg="dash_server_RL.py", fnc=fnc, msg=msg)
    ...
    class Request_Handler(BaseHTTPRequestHandler):
        ...
        def do_POST(self):
            ...
            if ( 'pastThroughput' in post_data ):
                ...
                # print "Video bitrate: {}".format(VIDEO_BIT_RATE[post_data['lastquality']])
                # print "Max video bitrate: {}".format(float(np.max(VIDEO_BIT_RATE)))
                # print "Buffer: {}".format(post_data['buffer'])
                # print "Video chunk size: {}".format(float(video_chunk_size))
                # print "Video chunk fetch time: {}".format(float(video_chunk_fetch_time))
                # print "Next video chunk sizes: {}".format(np.array(next_video_chunk_sizes))
...
def run(server_class=HTTPServer, port=8555, log_file_path=LOG_FILE):
