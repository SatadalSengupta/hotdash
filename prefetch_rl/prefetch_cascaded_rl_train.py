...
RANDOM_SEED = 42
RAND_RANGE = 1000
SUMMARY_DIR = './results_prefetch_rl'
LOG_FILE = './results_prefetch_rl/log'
TEST_LOG_FOLDER = './test_results_prefetch_rl/'
TRAIN_TRACES = './cooked_traces_prefetch_rl/'

START_EPOCH = 500
NN_MODEL = './results_prefetch_rl/nn_model_ep_' + str(START_EPOCH) + '.ckpt'
# NN_MODEL = None
...
def testing(epoch, nn_model, log_file):
    ...
    # run test script
    os.system('python prefetch_cascaded_rl_test.py ' + nn_model)
...
def central_agent(net_params_queues, exp_queues):
    ...
    with tf.Session() as sess, open(LOG_FILE + '_test', 'wb') as test_log_file:
        ...
        while True:
            ...
            if epoch % ENTROPY_CHANGE_INTERVAL == 0:
                actor.set_entropy_weight()

            log_stats = "Epoch: {}, TD_loss: {}, Avg_reward: {}, Avg_entropy: {}"
            log_stats = log_stats.format(epoch, avg_td_loss, avg_reward, avg_entropy)
            print log_stats
...
def agent(agent_id, all_cooked_time, all_cooked_bw, net_params_queue, exp_queue):
    ...
    with tf.Session() as sess, open(LOG_FILE + '_agent_' + str(agent_id), 'wb') as log_file:
        ...
        action_vec[prefetch_decision] = 1 # Normal chunk action: [1,0]; Hotspot chunk action: [0,1]
        ...
        while True:  # experience video streaming forever
            ...
            reward_normal_br = (1 - was_hotspot_chunk) * (VIDEO_BIT_RATE[last_bit_rate] / M_IN_K) * 1.0
            ...
            log_file.write(str(time_stamp) + '\t' +
                           ...
                           str(delay) + '\t' +
                           str(prefetch_decision) + '\t' +
                           str(reward) + '\n')
            log_file.flush()

            # report experience to the coordinator
            if len(r_batch) >= TRAIN_SEQ_LEN or end_of_video:
                exp_queue.put([s_batch[1:],  # ignore the first chunk
...
#     post_data = {}
#     post_data['nextChunkSize'] = 1000
#     post_data['lastquality'] = 4
#     post_data['buffer'] = 60
#     post_data['lastRequest'] = 12
#     post_data['RebufferTime'] = 15
#     post_data['delay'] = 10
#     post_data['lastChunkSize'] = 2000
#     post_data['videoChunkCount'] = 15
#     post_data['nextVideoChunkIndex'] = 12

#     try:
#         abr_request = requests.post('http://10.5.20.129:9999', data=json.dumps(post_data))
#         if abr_request.status_code == 200:
#             print abr_request.text
#         else:
#             print abr_request.status_code
